$(document).ready(function () {
    ...
    function trivia() {
        ...
        function createQuestion(questions) {
            ...
            //hides all except first question:
            for (var k = questions.length - 1; k > 0; k--) {
                $('#' + k).hide();
            }
        }
        //grabs answer choices from questions array/ returns them to createQuestion():
        function Buttons(ary, qNum) {
            var answers = [];
            for (i = 0; i < ary.length; i++) {
                answers.push('<label><input type="Button" name="' + qNum + '" value="' + ary[i] + '">' + ary[i] + '</label>');
            }
            return answers.join(" ");
        }
        ...
        $(".next").click(function (event) {
            ...
            var userInput = $('input[name=' + qNum + ']:Button:clicked').val(); //takes selected answer
            if (counter > 1) {
